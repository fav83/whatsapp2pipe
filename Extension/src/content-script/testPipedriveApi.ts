...
export function exposePipedriveTestHelpers() {
  ...
  const testPipedriveApi = async (message: unknown) => {
    logger.log('[Test] Sending message:', message)
    try {
      const response = await chrome.runtime.sendMessage(message)
      logger.log('[Test] Received response:', response)
      return response
    } catch (error) {
  ...
  const helpers = {
    ...
    async lookupByPhone(phone: string) {
      logger.log(`[Test] Looking up phone: ${phone}`)
    ...
    async searchByName(name: string) {
      logger.log(`[Test] Searching name: ${name}`)
      return testPipedriveApi({
    ...
    async createPerson(name: string, phone: string, email?: string) {
      logger.log(`[Test] Creating person: ${name}`)
      return testPipedriveApi({
    ...
    async attachPhone(personId: number, phone: string) {
      logger.log(`[Test] Attaching phone to person ${personId}`)
      return testPipedriveApi({
    ...
    async checkAuth() {
      ...
      logger.log('[Test] Authenticated:', isAuthed)
      if (isAuthed) {
        logger.log('[Test] Verification code exists (length:', result.verification_code.length, ')')
      } else {
        logger.log('[Test] Not authenticated. Please sign in first.')
      }
      return isAuthed
    },
  }
  ...
  logger.log('✅ Pipedrive test helpers loaded!')
  logger.log('📝 Available commands:')
  logger.log('  - window.testPipedrive.checkAuth()')
  logger.log('  - window.testPipedrive.lookupByPhone("+48123456789")')
  logger.log('  - window.testPipedrive.searchByName("John")')
  logger.log(
    '  - window.testPipedrive.createPerson("Jane Doe", "+48123456789", "jane@example.com")'
  )
  logger.log('  - window.testPipedrive.attachPhone(123, "+48123456789")')
}
