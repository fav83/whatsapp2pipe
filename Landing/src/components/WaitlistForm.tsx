    ...
    return (
      <div className="text-center">
        <div className={`inline-flex items-center justify-center w-12 h-12 rounded-full mb-4 ${
          isHero ? 'bg-indigo/10' : 'bg-white/20'
        }`}>
          <svg
            className={`w-6 h-6 ${isHero ? 'text-indigo' : 'text-white'}`}
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            aria-hidden="true"
        ...
        <p className={`text-lg ${isHero ? 'text-black' : 'text-white'}`}>
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full max-w-md">
      ...
      <p className={`mt-3 text-sm text-center ${
        isCTA ? 'text-white/70' : 'text-gray-secondary'
      }`}>
        {isCTA
