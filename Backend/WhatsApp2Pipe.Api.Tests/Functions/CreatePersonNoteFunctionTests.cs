...
public class CreatePersonNoteFunctionTests
{
    private readonly Mock<ILogger<PipedriveNotesCreateFunction>> mockLogger;
    ...
    private readonly PipedriveNotesCreateFunction function;

    public CreatePersonNoteFunctionTests()
    {
        mockLogger = new Mock<ILogger<PipedriveNotesCreateFunction>>();
        ...
        function = new PipedriveNotesCreateFunction(
            mockLogger.Object,
    ...
    {
        // Arrange
        var request = CreateMockHttpRequest("Bearer test-code", new { personId = 123, content = "Test note content" });
        ...
        {
            Success = true,
            Data = new PipedriveNote { Id = 456, Content = "Test note content", PersonId = 123 }
        };

        mockPipedriveApiClient.Setup(c => c.CreateNoteAsync(session, "Test note content", 123, null))
    ...
    {
        // Arrange
        var request = CreateMockHttpRequest(null, new { personId = 123, content = "Test content" });
    ...
    {
        // Arrange
        var request = CreateMockHttpRequest("InvalidFormat", new { personId = 123, content = "Test content" });
    ...
    {
        // Arrange
        var request = CreateMockHttpRequest("Bearer test-code", new { personId = 123, content = "Test content" });
    ...
    [Fact]
    public async Task Run_MissingPersonId_ReturnsBadRequest()
    {
        // Arrange
        ...
        using var reader = new StreamReader(bodyStream);
        var body = await reader.ReadToEndAsync();
        Assert.Contains("PersonId must be greater than 0", body);
    }

    [Fact]
    public async Task Run_InvalidPersonId_ReturnsBadRequest()
    {
        // Arrange
        var request = CreateMockHttpRequest("Bearer test-code", new { personId = 0, content = "Test content" });
    ...
    {
        // Arrange
        var request = CreateMockHttpRequest("Bearer test-code", new { personId = 123 });
    ...
    {
        // Arrange
        var request = CreateMockHttpRequest("Bearer test-code", new { personId = 123, content = "" });
    ...
    public async Task Run_PipedriveReturnsNullData_ReturnsInternalServerError()
    {
        ...
        mockPipedriveApiClient.Setup(c => c.CreateNoteAsync(session, "Test content", 123, null))
            .ReturnsAsync(pipedriveNoteResponse);
    ...
    public async Task Run_LongContent_ReturnsCreated()
    {
        ...
        var request = CreateMockHttpRequest("Bearer test-code", new { personId = 123, content = longContent });
        ...
        {
            Success = true,
            Data = new PipedriveNote { Id = 456, Content = longContent, PersonId = 123 }
        };

        mockPipedriveApiClient.Setup(c => c.CreateNoteAsync(session, longContent, 123, null))
            .ReturnsAsync(pipedriveNoteResponse);
        ...
        // Assert
        Assert.Equal(HttpStatusCode.Created, response.StatusCode);
        mockPipedriveApiClient.Verify(c => c.CreateNoteAsync(session, longContent, 123, null), Times.Once);
    }
    ...
    public async Task Run_ContentWithSpecialCharacters_ReturnsCreated()
    {
        ...
        var request = CreateMockHttpRequest("Bearer test-code", new { personId = 123, content = specialContent });
        ...
        {
            Success = true,
            Data = new PipedriveNote { Id = 456, Content = specialContent, PersonId = 123 }
        };

        mockPipedriveApiClient.Setup(c => c.CreateNoteAsync(session, specialContent, 123, null))
            .ReturnsAsync(pipedriveNoteResponse);
        ...
        // Assert
        Assert.Equal(HttpStatusCode.Created, response.StatusCode);
        mockPipedriveApiClient.Verify(c => c.CreateNoteAsync(session, specialContent, 123, null), Times.Once);
    }
