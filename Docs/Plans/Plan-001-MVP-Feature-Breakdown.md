...
### Feature 24: User Feedback System (Draft - Spec-124)
Implement in-extension feedback submission for authenticated users to collect bug reports, feature requests, and general feedback. Fixed "Send Feedback" button positioned at bottom of sidebar (above dev indicator in dev mode, at absolute bottom in production). Clicking opens centered modal overlay with simple textarea form (5000 character UI limit, 10000 character database capacity). Four form states: default (submit disabled when empty), submitting (spinner + disabled inputs), success (thank you message with close button), and error (banner with preserved text for retry). Backend: new Feedback table in Azure SQL (FeedbackEntityId GUID, UserId GUID, Message NVARCHAR(10000), CreatedAt, UserAgent, ExtensionVersion) and POST /api/feedback endpoint (authenticated, auto-populates metadata from token and headers). Extension components: FeedbackButton.tsx (fixed button) and FeedbackModal.tsx (modal with form states). Full keyboard navigation (Tab, Escape, Enter), focus trap, ARIA labels, and click-outside-to-close with confirmation. Follows existing UI patterns (button styles, modal backdrop, error handling) and API service layer (pipedriveApi.ts).

### Feature 25: Admin Config Message Banner (Draft - Spec-125)
Implement admin-configurable message banner system to display announcements, updates, or notifications to authenticated users at the top of the sidebar. Backend: new GET /api/config endpoint (authenticated) returns JSON `{ "message": string | null }` with Markdown content supporting inline HTML. Message stored as configuration setting (environment variable or hard-coded for MVP). Extension: ConfigMessageBanner.tsx component positioned below fixed header (logo + avatar), above all scrollable content. Visible across ALL sidebar states (welcome, contact, person-matched, etc.). Full sidebar width (350px minus padding), dismissible with X button in top-right corner. Background: light info color (bg-blue-50) with border. Markdown rendered using react-markdown with rehype-raw (HTML support) and rehype-sanitize (security). Links can specify target via `<a target="_blank">` tags. Dismissal is temporary UI state only (no persistence). Message reappears on next sidebar session (page reload). Config endpoint called once on initial load for authenticated users. API failures silent (logged to Sentry, sidebar continues normally). Sanitization allows safe HTML tags only: `<p>`, `<a>`, `<strong>`, `<em>`, `<br>`, `<ul>`, `<ol>`, `<li>`, `<b>`, `<i>`. NPM dependencies: react-markdown (^9.0.0), rehype-raw (^7.0.0), rehype-sanitize (^6.0.0).
